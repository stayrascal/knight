package com.knight.core.web.view;
...
public class OperationResult {
    ...
    //全局未知错误标识代码
    public final static String FAILURE = "99999";
    ...
    public enum OPERATION_RESULT_TYPE {
        ...
        @MetaData(value = "失败", comments = "操作处理失败。前端一般是红色的长时间或者需要用户主动关闭的气泡提示")
        FAILURE,

        @MetaData(value = "确认", comments = "本次提交中止，反馈用户进行确认。前端一般会弹出一个供用户‘确认’操作的对话框，然后用户主动确认之后自动再次发起请求并跳过确认检查进行后续业务处理")
        CONFIRM
    }

    /*返回success或者failure操作标识*/
    private String type;
    ...
    public static OperationResult buildConfirmResult(String message) {
        return new OperationResult(OPERATION_RESULT_TYPE.CONFIRM, message).setCode(SUCCESS);
    }
